...
This document provides a detailed breakdown of every component in the Schemo repository. Schemo is designed as a zero-friction, cloud-native engineering visualization platform for AI assistants.
...
The core of Schemo. A single file that serves both Claude (MCP/SSE) and ChatGPT (REST API).
...
| `@mcp.tool(name="plot")` | Claude MCP tool - generates control system plots |
| `@mcp.tool(name="circuit")` | Claude MCP tool - renders circuit schematics |
...
The server configures `TransportSecuritySettings` with explicit `allowed_hosts` for the production domain. Without this, the MCP SDK's DNS rebinding protection rejects requests with non-localhost Host headers (causing 421 errors).

**Tool naming:**
Tools use `@mcp.tool(name="plot")` instead of inheriting the Python function name. This makes them display as "Plot" under the "Schemo" integration in Claude Desktop, rather than the verbose "Render system plot".

**Plot type handling:**
The `plot_type` parameter includes a null guard because Claude sometimes sends `null` instead of a valid enum string for optional parameters.

### `chatgpt_widget.py` - ChatGPT iframe Bridge

Registers a `ui://dashboard/{plot_type}/{num}/{den}` MCP Resource. When ChatGPT requests this resource, it returns an HTML wrapper containing an iframe that loads the Cloudflare Pages dashboard. This satisfies ChatGPT's UI widget security model while keeping frontend code on Cloudflare.

### `circuit_renderer.py` - Circuit Schematic Engine

Interfaces with the `schemdraw` library to convert a list of `CircuitElement` objects (type, start, end, label) into a PNG image. Supports: resistor, capacitor, inductor, diode, voltage source, current source, ground, and wire.

---

## 2. Frontend: React Dashboard (`widget/`)

An interactive engineering dashboard for exploring control system plots. Hosted on Cloudflare Pages at `schemo.shaniai.tech`.
...
| Math rendering | KaTeX renders H(s) as proper LaTeX fraction with superscripts |
| Plot computation | All math computed client-side via `control.ts` (no backend calls) |
| Plot rendering | Plotly.js interactive charts with dark theme |
| Tab navigation | Switch between Bode, Step, Impulse, Nyquist, Root Locus |

### `widget/src/lib/control.ts` - Client-Side Math Engine

Pure TypeScript control systems library with zero dependencies:
...
> **Important:** The API subdomain MUST use "DNS Only" mode (grey cloud). Cloudflare's proxy buffers SSE connections and kills long-running streams, breaking the MCP protocol.
...
The `supergateway` package acts as a local SSE-to-stdio bridge. Claude Desktop spawns it as a subprocess, and it maintains a persistent SSE connection to the cloud backend.

> **Note:** The previous approach using `@modelcontextprotocol/inspector` was abandoned because it outputs non-JSON diagnostic text to stdout, breaking the MCP JSON-RPC protocol.
